      ...
      <header className={`sticky-header ${isScrolled ? 'scrolled' : ''} bg-transparent shadow-md transition-all duration-300`}>
        <div className="container mx-auto px-4 py-4 flex justify-between items-center relative z-50">
          <div className="flex items-center">
            ...
            {/* Mostrar "Mufasa Inmobiliaria" en móvil y desktop, pero el logo solo en desktop */}
            <Link to="/" onClick={() => setCurrentPage && setCurrentPage('home')} className="flex items-center space-x-2">
              {/* Texto "Mufasa Inmobiliaria" visible en todas las pantallas */}
              <span className="text-xl font-bold bg-gradient-to-r from-orange-500 to-red-500 bg-clip-text text-transparent hover:from-orange-600 hover:to-red-600 transition-colors duration-300">
                Mufasa Inmobiliaria
              </span>
              {/* Logo solo visible en pantallas grandes */}
              <img src="/assets/logo-1.webp" alt="Logo Mufasa" className="h-10 w-auto logo hidden sm:block" />
            </Link>
          </div>
          ...
          <div className="flex items-center space-x-4">
            <form onSubmit={handleSearchSubmit} className="relative hidden sm:block">
              ...
              <button
                type="submit"
                className="absolute right-0 top-0 mt-2 mr-2 bg-[#FF6B35] text-black px-2 py-1 rounded-full hover:bg-[#E34A15] transition duration-300"
              >
        ...
        {/* Fondo oscuro para el menú de hamburguesa */}
        <div
        ...
        {/* Menú lateral */}
        <div
          className={`fixed inset-y-0 left-0 w-64 bg-black shadow-lg transform transition-transform duration-300 ease-in-out z-50 ${
        ...
        >
          ...
          <nav className="p-5 space-y-4">
            <Link
              ...
              className="block text-white hover:text-[#FF6B35]"
            >
              Inicio
            </Link>
            <Link
              to="/properties"
              onClick={() => {
                setCurrentPage && setCurrentPage('properties');
                handleToggleMenu();
              }}
              className="block text-white hover:text-[#FF6B35]"
            >
            ...
            <Link
              to="/offices"
              onClick={() => {
                setCurrentPage && setCurrentPage('offices');
                handleToggleMenu();
              }}
              className="block text-white hover:text-[#FF6B35]"
            >
            ...
            <Link
              to="/advisors"
              onClick={() => {
                setCurrentPage && setCurrentPage('advisors');
                handleToggleMenu();
              }}
              className="block text-white hover:text-[#FF6B35]"
            >
            ...
            <Link
              to="/about"
              onClick={() => {
                setCurrentPage && setCurrentPage('about');
                handleToggleMenu();
              }}
              className="block text-white hover:text-[#FF6B35]"
            >
